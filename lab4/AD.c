...
typedef struct {
    ...
    unsigned int  parent;
    unsigned int  keys[2 * B_TREE_DEGREE];
    unsigned int  children[2 * B_TREE_DEGREE + 1];
...
void              NFunction(TVector *str, unsigned int *nVector);
void              ZFunction(const TVector *string, unsigned int *zVector);
unsigned int      *CreateGoodSuffTable(TVector *str, const unsigned int* nVector);
...
unsigned int      *CreateArr(size_t size);
void              DestroyArr(unsigned int *arr);

int main(int argc, char *argv[]) {
    TVector text    = VectorCreate(100);
    ...
    // preprocessing for B-M algo
    TTree badCharTable = TreeCreate();
    FillBadCharTable(&badCharTable, &pattern);
    unsigned int *nVector = CreateArr(pattern.size);
    NFunction(&pattern, nVector);
    unsigned int *goodSuffTable = CreateGoodSuffTable(&pattern, nVector);
    unsigned int *mVector = CreateArr(text.size);
    ...
    while (i <= textLen - patternLen) {
        j = patternLen - 1;
        while (j >= 0) {
            if (mVector[i + j] == NONE || mVector[i + j] == 0) {
                if (strcmp(text.value[i + j].key, pattern.value[j].key) == 0) {
                    if (mVector[i + patternLen - 1] == NONE) {
                        mVector[i + patternLen - 1] = 0;
                    }
                    ++mVector[i + patternLen - 1];
                    if (j == 0) {
                        mVector[i + patternLen - 1] = patternLen;
                    }
                    --j;
                } else {
                    mVector[i + patternLen - 1] = (patternLen - 1) - (j + 1);
                    break;
                }
            } else {
                if (mVector[i + j] == nVector[j] && nVector[j] == j) {
                    mVector[i + patternLen - 1] = patternLen - 1 - j;
                } else if (mVector[i + j] > nVector[j]) {
                    mVector[i + patternLen - 1] = patternLen - 1 - j;
                    if (nVector[j] < j) {
                        break;
                    }
...
unsigned int *CreateArr(size_t size) {
    ...
    if (arr == NULL) {
        fprintf(stderr, "ERROR: could not allocate memory for array at line %d\n", __LINE__);
        exit(1);
    }
    return arr;
}
...
unsigned int *CreateGoodSuffTable(TVector *str, const unsigned int* nVector) {
    ...
    unsigned int *lVector = CreateArr(patternLen);
    if (lVector == NULL) {
...
void TreeSplitChild(TTree* tree, unsigned int parentInd, unsigned int pos) {
    ...
    new_child.keys_num = B_TREE_DEGREE - 1;
    for (size_t i = 0; i < B_TREE_DEGREE - 1; ++i) {
        new_child.keys[i] = child->keys[i + B_TREE_DEGREE];
    }
    if (child->distToBottom != 0) {
        for (size_t i = 0; i < B_TREE_DEGREE; ++i) {
            new_child.children[i] = child->children[i + B_TREE_DEGREE];
            child->children[i + B_TREE_DEGREE] = 0;
        }
    }
    child->keys_num = B_TREE_DEGREE - 1;
    ...
    parent->keys[pos] = child->keys[B_TREE_DEGREE - 1];
    ++(parent->keys_num);
}
...
void Insert(TTree* tree, unsigned int node, const TElem* elem) {
    ...
    if (tree->nMemory.storage[node].distToBottom == 0) {
        ...
        if (tree->nMemory.storage[child].keys_num == 2 * B_TREE_DEGREE - 1) {
...
int TreeInsert(TTree* tree, const TElem* elem) {
    ...
    if (tree->nMemory.storage[root].keys_num == 2 * B_TREE_DEGREE - 1) {
        TNode newRoot = {0};
...
void MergeNodes(TTree *tree, unsigned int parentInd, int pos) {
    ...
    if (rightC->distToBottom) {
        for (size_t i = 0; i <= rightC->keys_num; ++i) {
            leftC->children[B_TREE_DEGREE + i] = rightC->children[i];
            rightC->children[i] = 0;
        }
    }
    rightC->keys_num = 0;
...
void Rebalance(TTree *tree, unsigned int nodeInd, int* pos) {
    ...
    TNode *child = &(tree->nMemory.storage[childInd]);
    if ((*pos) >= 1 && tree->nMemory.storage[node->children[(*pos) - 1]].keys_num >= B_TREE_DEGREE) {
        ...
        --(leftN->keys_num);
    } else if ((*pos) + 1 <= node->keys_num && tree->nMemory.storage[node->children[(*pos) + 1]].keys_num >= B_TREE_DEGREE) {
        unsigned int ind = node->children[(*pos) + 1];
    ...
    } else if ((*pos) >= 1 && tree->nMemory.storage[node->children[(*pos) - 1]].keys_num == B_TREE_DEGREE - 1) {
        MergeNodes(tree, nodeInd, (*pos) - 1);
        --(*pos);
    } else if ((*pos) + 1 <= node->keys_num && tree->nMemory.storage[node->children[(*pos) + 1]].keys_num == B_TREE_DEGREE - 1) {
...
void RemoveNode(TTree *tree, unsigned int node, const char *str) {
    ...
    if (pos >= 0 && cur->distToBottom) {
        ...
        if (leftC->keys_num >= B_TREE_DEGREE) {
        ...
        if (rightC->keys_num >= B_TREE_DEGREE) {
    ...
    pos = -pos - 1;
    if (tree->nMemory.storage[cur->children[pos]].keys_num == B_TREE_DEGREE - 1) {
...
/* TODO: use Unordered map instead B-tree */
