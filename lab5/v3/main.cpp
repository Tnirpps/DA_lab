...
private:
    ...
    // start finst string s[begin:s.size()] in tree
    size_t find(size_t begin, const string& s, TState& st) {
        int n = s.size();
        int i = begin;
        while (i < n) {
            if (!has_way(st, s[i])) {
    ...
    // Return deph of finit state
    size_t find(const string& s, TState& st) {
        return find(0, s, st);
    }
...
public:
    ...
    vector<size_t> find_occurrences(const string& s) {
        ...
        st.node = root;
        vector<size_t> res;
        if (find(s, st) != s.size()) return res;
        if (st.on_edge) {
            st.node = edges[st.edge].dest_node;
        }
        // BFS
    ...
    vector<size_t> match_statistics(const string& s) {
        TState st;
        st.node = root;
        vector<size_t> res(s.size());
        res[0] = find(0, s, st);
        for (int i = 1; i < s.size(); ++i) {
            if (st.on_edge) {
                size_t char_ind = edges[st.edge].begin_index;
                int depth = st.pos_on_edge + 1;
                st.on_edge = false;
                if (st.node == root) {
                    ++char_ind;
                    --depth;
                ...
                go_suff_link(st);
                go_down(st, char_ind, depth);
                assert(res[i - 1] != 0);
                res[i] = res[i - 1] - 1 + find(i + res[i - 1] - 1, s, st);
            } else {
                if (st.node == root) {
                    res[i] = find(i, s, st);
                } else {
...
int main() {
    ...
    cin >> text;
    vector<size_t> v = tree.match_statistics(text);
    for (size_t i = 0; i < v.size(); ++i) {
        if (v[i] + 1 == pattern.size()) {
            cout << i + 1 << '\n';
        }
    }
    return 0;
