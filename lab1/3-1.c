...
int main(int argc, char *argv[]) {
    ...
    char str[66] = {0};
    int strlen;
    while (_scanUINT(&x.key) != EOF &&  ((strlen=_scanSTR(&str[1])) != EOF)) {
        str[0] = strlen;
        strlen++;
    ...
    for (size_t i = 0; i < v.size; ++i) {
        _printUINT(v.value[i].key);
        _printCHR('\t');
        s = (int) string[v.value[i].data];
        for (size_t j = 1; j <= s; ++j) {
            _printCHR(string[v.value[i].data + j]);
        }
        _printCHR('\n');
    }
